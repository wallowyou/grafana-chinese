...
func (hs *HTTPServer) setIndexViewData(c *m.ReqContext) (*dtos.IndexViewData, error) {
	...
	if hasEditPermissionInFoldersQuery.Result {
		...
		if c.OrgRole == m.ROLE_ADMIN || c.OrgRole == m.ROLE_EDITOR {
			...
			children = append(children, &dtos.NavLink{Text: "文件夹", SubTitle: "创建一个新文件夹来组织您的仪表板", Id: "folder", Icon: "gicon gicon-folder-new", Url: setting.AppSubUrl + "/dashboards/folder/new"})
		}
	...
	dashboardChildNavs := []*dtos.NavLink{
		{Text: "主页", Id: "home", Url: setting.AppSubUrl + "/", Icon: "gicon gicon-home", HideFromTabs: true},
		{Text: "分割", Divider: true, Id: "divider", HideFromTabs: true},
		{Text: "管理", Id: "manage-dashboards", Url: setting.AppSubUrl + "/dashboards", Icon: "gicon gicon-manage"},
		{Text: "播放清单", Id: "playlists", Url: setting.AppSubUrl + "/playlists", Icon: "gicon gicon-playlists"},
	...
	if c.IsSignedIn {
		...
		if !setting.DisableSignoutMenu {
			...
			// })
			profileNode.Children = append(profileNode.Children, &dtos.NavLink{
				Text: "登出", Id: "sign-out", Url: setting.AppSubUrl + "/logout", Icon: "fa fa-fw fa-sign-out", Target: "_self",
			})
		}
	...
	for _, plugin := range enabledPlugins.Apps {
		if plugin.Pinned {
			...
			if len(appLink.Children) > 0 && c.OrgRole == m.ROLE_ADMIN {
				...
				appLink.Children = append(appLink.Children, &dtos.NavLink{Text: "插件配置", Icon: "gicon gicon-cog", Url: setting.AppSubUrl + "/plugins/" + plugin.Id + "/"})
			}
